...
{
    public partial class ucPhongBan : DevExpress.XtraEditors.XtraForm
    {
        public ucPhongBan()
        {
            InitializeComponent();
        }
        Class.csDVXuLy CongVan = new Class.csDVXuLy();
        void Refesh()
        {
            try
            {
                grcData.DataSource = CongVan.SELECT_DVXULY();
            }
            catch
            {
            }
        ...
        private void txtMa_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                bool TrangThai = CongVan.SELECT_DVXULY_ID(ID);
                if (TrangThai == true)
                {
                    bLuu.Enabled = false;
        ...
        private void btXoa_Click(object sender, EventArgs e)
        {
            try
            {
                if (ThamSoHeThong.Quyen("ucPhongBan", "sDelete") == false)
                    return;
                if (DevExpress.XtraEditors.XtraMessageBox.Show("Bạn thật sự muốn xóa ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    ...
                    bool TrangThaiDen = CongVan.CHECK_CANBOXULY(ID);
        ...
        {
            if (ThamSoHeThong.Quyen("ucPhongBan", "sPrint") == false)
                return;
        ...
        {
            if (ThamSoHeThong.Quyen("ucPhongBan", "sExport") == false)
                return;
        ...
        private void bLuu_Click(object sender, EventArgs e)
        {
            try
            {
                if ((txtMa.Properties.ReadOnly == false) && (ThamSoHeThong.Quyen("ucPhongBan", "sAdd") == false))
                    return;
                if ((txtMa.Properties.ReadOnly == true) && (ThamSoHeThong.Quyen("ucPhongBan", "sEdit") == false))
                    return;
                if (txtMa.Text == "")
                {
                ...
                else
                {
                    ...
                    {
                        CongVan.INSERT_DVXULY(ID, Name, Description, Active);
                    }
                    else
                    {
                        CongVan.UPDATE_DVXULY(ID, Name, Description, Active);
